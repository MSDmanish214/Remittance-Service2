...
export const CONTRACT_ADDRESS =
  "CDVLL3BCEOCVAS7W7XYUJ3QDNL7RMC2N45QP2I47OXE5KXFCOKDOEG";
...
export function toScValI64(value: number): xdr.ScVal {
  return nativeToScVal(value, { type: "i64" });
}

export function toScValTupleI128U64(value: [bigint, number]): xdr.ScVal {
  return nativeToScVal(value, { type: { tuple: ["i128", "u64"] } });
}

export function toScValTupleI128(value: [bigint]): xdr.ScVal {
  return nativeToScVal(value, { type: { tuple: ["i128"] } });
}

// ============================================================
// Remittance Service — Contract Methods
// ============================================================

/**
 * Send a remittance.
 * Calls: send(sender: Address, recipient: Address, secret: String, params: (i128, u64)) -> bool
 */
export async function sendRemittance(
  ...
  return callContract(
    ...
    [
      ...
      toScValString(secret),
      toScValTupleI128U64([amount, expirySeconds]),
    ],
    sender,
    true
  );
...
/**
 * Claim a remittance.
 * Calls: claim(recipient: Address, secret: String, sender: Address, params: (i128,)) -> bool
 */
export async function claimRemittance(
  ...
  return callContract(
    ...
    [
      ...
      toScValString(secret),
      toScValAddress(sender),
      toScValTupleI128([claimedAmount]),
    ],
    recipient,
    true
  );
...
/**
 * Cancel an expired remittance.
 * Calls: cancel(sender: Address, recipient: Address, secret: String) -> bool
 */
 ...
 * Calls: get_remittance(sender: Address, recipient: Address, secret: String) -> Remittance
 * Returns: { sender, recipient, secret, amount, claimed, expiry, is_claimed, is_cancelled }
 */
